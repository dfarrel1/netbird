...
func offer(t *testing.T, key []byte, remoteFeatures []uint32) *sigproto.Message {
	...
	m, err := MarshalCredentialFor(k, 51820, r.PublicKey().String(),
		&Credential{UFrag: "abcd1234", Pwd: "0123456789abcdef0123456789abcdef"},
		sigproto.Body_OFFER, key, "100.64.0.5:9999", "rels://relay.example.net:443",
		[]byte("session-id-16byt"), remoteFeatures)
	if err != nil {
		t.Fatal(err)
	}
	return m
